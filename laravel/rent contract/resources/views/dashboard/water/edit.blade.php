...
@section('title', 'تعديل المياه')
@section('content')
    <div class="container-fluid">
        <div class="card mt-30 mb-50">
            <div class="card-header color-dark fw-500">
                <h4 class="text-capitalize">تعديل المياه</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('waters.update', $water->id) }}" enctype="multipart/form-data" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="row">
                            ...
                            <select name="contractNum" id="contractNum"
                                ...
                                @foreach ($contracts as $data)
                                    <option value="{{ $data->id }}"
                                        {{ $water->contractNum == $data->id ? 'selected' : '' }}>
                                        {{ $data->buildingNum }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <br>
                    <button type="submit" class="btn btn-primary">تحديث المياه</button>
                </form>
            </div>
        </div>
    </div>
